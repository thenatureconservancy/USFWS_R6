...
bps_data <- read.csv("data/rocky_basin/bps_aoi_attributes.csv")
...
evt_data <- read_csv(file = "data/rocky_basin/evt_aoi_attributes.csv")

evc_data <- read.csv("data/rocky_basin/evc_aoi_attributes.csv") 

evh_data <- read.csv("data/rocky_basin/evh_aoi_attributes.csv") 
...
::: {.callout-note}
[ **Prior to European colonization the fee tracts within the FWS Rocky Basin fire zone had `r formatC(sum(all_annual_fire$acres), format = "f", big.mark = ",", digits = 0)` acres of fire annually.** ]{style="font-size: 1.5em;"}
:::
...
additional_shape <- st_read("data/rocky_basin/rocky_basin_tracts.shp", quiet = TRUE)
